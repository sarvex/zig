...
class ArrayListPrinter:
    ...
    def to_string(self):
        ...
        return f"{type} of length {self.val['items']['len']}, capacity {self.val['capacity']}"
...
class MultiArrayListPrinter:
    ...
    def child_type(self):
        (helper_fn, _) = gdb.lookup_symbol(f'{self.val.type.name}.dbHelper')
        return helper_fn.type.fields()[1].type.target()

    def to_string(self):
        type = self.val.type.name[len('std.multi_array_list.'):]
        return f"{type} of length {self.val['len']}, capacity {self.val['capacity']}"
...
class HashMapPrinter:
    ...
    def to_string(self):
        ...
        hdr = self.header()
        cap = hdr['capacity'] if hdr is not None else 0
        return f"{type} of length {self.val['size']}, capacity {cap}"
...
class ArrayHashMapPrinter:
    ...
    def to_string(self):
        ...
        return f"{type} of length {self.val['entries']['len']}"

    def children(self):
        entries = MultiArrayListPrinter(self.val['entries'])
        len = self.val['entries']['len']
        fields = {str(name): ptr for name, ptr in entries.slice()}
        for i in range(len):
            if 'key' in fields:
